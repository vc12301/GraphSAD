...
class Model(nn.Module):
    def __init__(self, args, device, eps=1., **kwargs):
        ...
        self.semantic_GNN = GNN(self.edge_input_dim, self.hidden_dim, self.num_layer, flow="target_to_source")
    ...
    def forward(self, semantic_data, temporal_data, node_index=None):
        x = semantic_data.x
        r = self.tcn(x)
        ...
        z = self.proj(z)
        out1 = self.semantic_GNN(semantic_data, z)
        out2 = self.temporal_GNN(temporal_data, z)
        out = torch.cat([out1, out2], -1)
        context1 = self.context(out, semantic_data.edge_index)
        context2 = self.context(out, temporal_data.edge_index)
        x_hat = self.decoder(out)
        length_penalty = self.laplacian_regularization(semantic_data, temporal_data, distr)
        decode_loss = self.decode_regularization(x, x_hat)
        return out - context1, out - context2, length_penalty, decode_loss, out
    ...
    def laplacian_regularization(self, semantic_data, temporal_data, distr):
        semantic_edge_index, semantic_edge_attr = to_undirected(semantic_data.edge_index, edge_attr=semantic_data.edge_attr, reduce='max')
        ...
        semantic_adj = to_dense_adj(semantic_edge_index).squeeze(0).unsqueeze(-1)
